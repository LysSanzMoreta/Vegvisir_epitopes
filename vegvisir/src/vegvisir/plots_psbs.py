#### Extracted from https://github.com/dvgodoy/PyTorchStepByStep/blob/master/plots/chapterextra.py #####
...
def get_plot_data(train_loader, n_layers=5, hidden_units=100, activation_fn=None, use_bn=False, before=True,
                  model=None):
    import sys
    sys.path.append('..')
...
def add_arrow(line, position=None, direction='right', size=15, color=None, lw=2, alpha=1.0, text=None,
              text_offset=(0, 0)):
    """
    ...
    line.axes.annotate('',
                       ...
                       arrowprops=dict(arrowstyle="->", color=color, lw=lw, linestyle='--' if alpha < 1 else '-',
                                       alpha=alpha),
                       size=size,
                       )
    if text is not None:
