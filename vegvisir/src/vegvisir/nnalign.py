...
def run_nnalign(args,storage_folder,script_dir= "/home/projects/vaccine/people/lyssan/vegvisir_epitopes"):
    ...
    for motif_len in range(3,6):
        #TODO: Otherwise use subprocess.run()
        p = subprocess.Popen(args=[nnalign_location,"-f",train_data,"-name","nnalign_viral3", "-testset",test_data, "-Logo", seq2logo_location,
                               "-lgt",str(motif_len),"-nh", str(80), "-split",str(3),"-encoding",str(2),"-ishort","-fl",str(0),"-stop","-iter",str(250),"-procs",str(2),"-nh",str(80)],
                               cwd=working_dir)
