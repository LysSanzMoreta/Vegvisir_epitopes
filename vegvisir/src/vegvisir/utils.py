...
class SequenceRandomGeneration(object):
    ...
    def run(self):
        ...
        if self.padding_method not in padded_sequences.keys():
            raise ValueError("Padding method <{}> not implemented, please choose among <{}>".format(self.padding_method,padded_sequences.keys()))
        else:
            return padded_sequences[self.padding_method]

    def no_padding(self,seq,seed,max_len):
        print("Here1")
        np.random.seed(seed)
        seq = self.aminoacids_list[np.random.choice(len(seq),0)]
        print(seq)
        exit()
        seq = "".join(seq)
        return (seq,seq)
    ...
    def random_padding(self,seq,seed, max_len):
        ...
        np.random.seed(seed)
        seq = np.random.choice(self.aminoacids_list,len(seq))
        #seq = "".join(seq)
        pad = max_len - len(seq)
        seq = list(seq)
        if pad != 0:
...
def generate_random_sequences(data,sequences_list,seq_max_len,args,unique_lens,storage_folder):
    ...
    if len(unique_lens) > 1: # Highlight: Pad the sequences (relevant when they differ in length)
        ...
        sequences_pad_result = SequenceRandomGeneration(sequences_list,seq_max_len,args.seq_padding).run()
    else:
        sequences_pad_result = SequenceRandomGeneration(sequences_list,seq_max_len,"no_padding").run()

    sequences_padded, sequences_padded_mask = zip(*sequences_pad_result)
