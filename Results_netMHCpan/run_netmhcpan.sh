...
echo $foldername
mkdir -p "${results}/${foldername}" #does not create a new folder if exists
...
do
  ...
  echo "$results/$foldername/group${counter}_alleles.txt"
  netmhcpan -p $peptides -a $line -l , > "$results/$foldername/group${counter}_alleles"
  counter=$((counter+=1))
done